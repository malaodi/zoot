"""Read-only helpers for Zoot run/session evidence.

These helpers intentionally do not import or instantiate the Zoot runtime. They
only interpret files that a completed CLI/REPL/TUI run left under `.zoot/`.
"""
...
class RunEvidence:
    ...
    def latest(cls, workspace: Path) -> "RunEvidence":
        ...
        run_dir = _latest_dir(workspace / ".zoot" / "runs")
        ...
        session_path = _latest_file(workspace / ".zoot" / "sessions", "*.json")
        session_event_path = _latest_file(workspace / ".zoot" / "sessions", "*.events.jsonl")
        return cls(
            workspace=workspace,
            run_dir=run_dir,
