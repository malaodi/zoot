...
这个模块负责把“用户怎么启动 zoot”翻译成 runtime 能理解的对象：
...
SECRET_ENV_NAMES_VAR = "ZOOT_SECRET_ENV_NAMES"
...
def build_agent(args):
    ...
    - 输出：一个新的 `Zoot`，或一个从旧 session 恢复出来的 `Zoot`
...
def build_arg_parser():
    ...
    parser.add_argument(
        "--config", default=None, help="Path to a Zoot TOML config file."
    )
    parser.add_argument(
        "--provider",
        default=None,
    ...
    parser.add_argument(
        "--memory-dir",
        default=None,
        help="Memory directory. Defaults to .zoot/memory in the workspace.",
    )
    parser.add_argument(
        "--no-auto-dream",
        action="store_true",
...
def main(argv=None):
    ...
    if mode == "tui":
        from .tui.app import ZootTuiApp
