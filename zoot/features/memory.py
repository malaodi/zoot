...
You are performing a dream: a reflective pass over Zoot's memory files. Synthesize recent signal into durable, well-organized memory files so future sessions can orient quickly.
...
def maintain_memory_after_turn(agent, final_answer):
    ...
    thread = threading.Thread(target=_background_dream, name="zoot-auto-dream", daemon=True)
...
def normalize_memory_state(state, workspace_root=None):
    ...
    durable_root = Path(workspace_root) / ".zoot" / "memory" if workspace_root is not None else None
...
class LayeredMemory:
    def __init__(self, state=None, workspace_root=None):
        ...
        self.durable_store = DurableMemoryStore(Path(workspace_root) / ".zoot" / "memory") if workspace_root is not None else None
