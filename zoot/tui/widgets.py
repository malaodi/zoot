...
class WelcomeBanner(Static):
    ...
    def render(self) -> Text:
        ...
        rows = [
            Text.assemble(
                Text("zoot", style=f"bold {accent}"),
                Text("  local coding agent", style=muted),
            ),
            Text(""),
        ]
        rows.extend(Text(line, style=accent) for line in ZOOT_MARK)
        rows.extend(
            [
                Text(""),
                Text.assemble(
...
class AssistantMessage(Static):
    ...
    def compose(self):
        yield Static("zoot", classes="message-label")
...
class InputBar(Static):
    ...
    def __init__(self) -> None:
        super().__init__()
        self.input = Input(placeholder="Ask zoot or type /help")
    ...
    def set_busy(self, busy: bool) -> None:
        ...
        self.input.placeholder = (
            "zoot is working..." if busy else "Ask zoot or type /help"
        )
