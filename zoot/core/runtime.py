...
Zoot owns session state, workspace context, memory, checkpoints, and persistence.
...
class Zoot(RuntimeSecretsMixin, RuntimeCheckpointsMixin):
    def __init__(
        self,
        model_client,
        workspace,
        ...
        self.run_store = run_store or RunStore(
            Path(workspace.repo_root) / ".zoot" / "runs"
        )
    ...
    def build_prefix(self):
        ...
        text = textwrap.dedent(
            f"""\
            You are zoot, a small local coding agent working inside a local repository.
