...
_PLAN_DIR_MARKER = "/.zoot/plans/"


def _plan_path(topic, path=None):
    if path:
        value = str(path).strip()
        # 模型有时给绝对路径，如 /Users/u/repo/.zoot/plans/foo；自动把它相对化。
        if value.startswith("/") and _PLAN_DIR_MARKER in value:
            value = value[value.index(_PLAN_DIR_MARKER) + 1 :]
        if value.startswith("./"):
            value = value[2:]
    else:
        value = f".zoot/plans/{_slug(topic)}-plan.md"
    if (
        not value.startswith(".zoot/plans/")
        or value.endswith("/")
        or ".." in value.split("/")
    ):
        raise ValueError("plan path must stay under .zoot/plans/")
    return value
