...
REPO="${ZOOT_REPO:-https://github.com/martin-los/zoot.git}"
INSTALL_DIR="${ZOOT_INSTALL_DIR:-$HOME/.zoot-agent}"
BRANCH="${ZOOT_BRANCH:-main}"

RED='\033[0;31m'; GREEN='\033[0;32m'; YELLOW='\033[1;33m'; CYAN='\033[0;36m'
BOLD='\033[1m'; RESET='\033[0m'
info()    { printf "${CYAN}[zoot]${RESET} %s\n" "$*"; }
success() { printf "${GREEN}[zoot]${RESET} ${BOLD}%s${RESET}\n" "$*"; }
warn()    { printf "${YELLOW}[zoot]${RESET} %s\n" "$*" >&2; }
die()     { printf "${RED}[zoot] ERROR:${RESET} %s\n" "$*" >&2; exit 1; }
...
main() {
  ...
  安装方法：sudo apt install python3.11   (Debian/Ubuntu)
    ...
    BIN_DIR="${ZOOT_BIN_DIR:-$HOME/.local/bin}"
    mkdir -p "$BIN_DIR"
    LAUNCHER="$BIN_DIR/zoot"

    cat > "$LAUNCHER" <<EOF
#!/usr/bin/env bash
exec "${VENV_DIR}/bin/zoot" "\$@"
EOF
    chmod +x "$LAUNCHER"
