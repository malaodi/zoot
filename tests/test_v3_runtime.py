...
def test_engine_drives_real_session_and_persists_event_timeline(tmp_path):
    ...
    assert (
        ...
        == tmp_path / ".zoot" / "sessions" / f"{agent.session['id']}.events.jsonl"
    )
...
def test_plan_mode_allows_only_the_active_plan_artifact_until_plan_is_written(tmp_path):
    agent = build_agent(
        tmp_path,
        [
            '<tool name="write_file" path=".zoot/plans/v3-plan.md"><content># Plan\n- Build Engine\n</content></tool>',
    ...
    assert plan_path == ".zoot/plans/v3-plan.md"
    ...
    assert (
        (tmp_path / ".zoot" / "plans" / "v3-plan.md")
...
def test_plan_mode_tools_enter_and_exit_runtime_mode(tmp_path):
    ...
    assert ".zoot/plans/refactor-auth-plan.md" in entered
    assert agent.runtime_mode == "plan"
...
def test_plan_path_accepts_absolute_path_inside_workspace(tmp_path):
    """模型偶尔给绝对路径，如 /Users/u/repo/.zoot/plans/foo —— 自动相对化，
    不应该让 agent 多走一次重试。"""
    from zoot.core.plan_mode import _plan_path

    assert (
        _plan_path("Student Mgmt", "/Users/u/repo/.zoot/plans/student-mgmt.md")
        == ".zoot/plans/student-mgmt.md"
    )
    assert (
        _plan_path("X", "./.zoot/plans/x-plan.md") == ".zoot/plans/x-plan.md"
    )
    # 真正越界的还是要拒
    import pytest
