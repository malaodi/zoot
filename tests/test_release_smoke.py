...
Set ZOOT_LIVE_SMOKE=1 with a provider configured to run them against a real model.
"""
import os
import textwrap
...
def test_read_then_edit_completes_in_one_turn(tmp_path):
    ...
    agent = _build_agent(
        ...
        [
            ...
            '<tool>{"name":"patch_file","args":{"path":"README.md","old_text":"Quick Start coming soon.","new_text":"Quick Start: zoot --help"}}</tool>',
            "<final>已把 Quick Start 段从占位文本改成实际的 zoot --help 提示。</final>",
        ],
    )
...
def test_empty_response_does_not_silently_stop(tmp_path):
    ...
    from zoot.providers.errors import ProviderError
...
def _has_live_provider():
    ...
    return any(os.environ.get(k) for k in keys) and os.environ.get("ZOOT_LIVE_SMOKE") == "1"


@pytest.mark.skipif(
    not _has_live_provider(),
    reason="set ZOOT_LIVE_SMOKE=1 and a provider API key to run against a real model",
)
def test_dream_produces_non_empty_topics_with_live_provider(tmp_path):
    """End-to-end: 真实 provider 跑一次 dream，topics/ 必须产出非空文件。"""
    from zoot.config import resolve_provider_config
    from zoot.providers import (
    ...
    log_path = tmp_path / ".zoot" / "memory" / "logs" / "2026" / "05" / "2026-05-13.md"
