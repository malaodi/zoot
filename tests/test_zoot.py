...
def build_agent(tmp_path, outputs, **kwargs):
    ...
    store = SessionStore(tmp_path / ".zoot" / "sessions")
...
def test_list_files_hides_internal_agent_state(tmp_path):
    agent = build_agent(tmp_path, [])
    (tmp_path / ".zoot").mkdir(exist_ok=True)
    (tmp_path / ".git").mkdir(exist_ok=True)
    (tmp_path / "hello.txt").write_text("hi\n", encoding="utf-8")

    result = agent.run_tool("list_files", {})

    assert ".zoot" not in result
...
def test_openai_compatible_client_posts_expected_responses_payload():
    ...
    assert captured["headers"]["User-agent"] == "zoot/0.1"
...
def test_openai_compatible_client_retries_rate_limit_and_records_retry_metadata():
    ...
    with patch("urllib.request.urlopen", fake_urlopen), patch("zoot.providers.clients.time.sleep"):
        result = client.complete("hello", 42)
...
def test_build_agent_uses_openai_provider_and_model_override(tmp_path):
    ...
    with patch.dict(
        ...
        with patch("zoot.cli.OpenAICompatibleModelClient") as mock_openai:
            fake_client = mock_openai.return_value
            agent = zoot_pkg.build_agent(args)
...
def test_build_arg_parser_leaves_provider_to_config_by_default(tmp_path):
    args = zoot_pkg.build_arg_parser().parse_args(["--cwd", str(tmp_path)])

    assert args.provider is None


def test_build_arg_parser_accepts_anthropic_provider(tmp_path):
    args = zoot_pkg.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--provider", "anthropic"])

    assert args.provider == "anthropic"


def test_build_arg_parser_accepts_deepseek_provider(tmp_path):
    args = zoot_pkg.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--provider", "deepseek"])
...
def test_build_agent_uses_anthropic_provider_and_openai_key_fallback(tmp_path):
    ...
    with patch.dict(
        ...
        with patch(
            "zoot.cli.OpenAICompatibleModelClient",
            side_effect=AssertionError("openai client should not be used"),
        ), patch("zoot.cli.AnthropicCompatibleModelClient") as mock_anthropic:
...
def test_build_agent_uses_anthropic_default_model_when_env_is_missing(tmp_path):
    ...
    with patch.dict(
        ...
        with patch("zoot.cli.AnthropicCompatibleModelClient") as mock_anthropic:
            zoot_pkg.build_agent(args)
...
def test_build_agent_uses_deepseek_provider_and_env_configuration(tmp_path):
    (tmp_path / ".env").write_text(
        "\n".join(
            [
                "ZOOT_DEEPSEEK_API_BASE=https://api.deepseek.com/anthropic",
                "ZOOT_DEEPSEEK_API_KEY=sk-project-deepseek",
                "ZOOT_DEEPSEEK_MODEL=deepseek-v4-pro",
            ]
        )
        + "\n",
        encoding="utf-8",
...
def test_build_agent_uses_provider_profile_protocol_from_project_toml(tmp_path):
    ...
    with patch.dict(os.environ, {"ZOOT_DEEPSEEK_API_KEY": "sk-legacy-env"}, clear=True):
        with patch(
            "zoot.cli.OpenAICompatibleModelClient",
            side_effect=AssertionError("openai client should not be used"),
        ), patch("zoot.cli.AnthropicCompatibleModelClient") as mock_anthropic:
...
def test_explicit_memory_promotion_persists_durable_memory_topics(tmp_path):
    agent = build_agent(
        ...
        [
            ...
            "Project convention: Preserve local agent state under .zoot/.\n"
    ...
    index_path = tmp_path / ".zoot" / "memory" / "MEMORY.md"
    conventions_path = tmp_path / ".zoot" / "memory" / "topics" / "project-conventions.md"
    decisions_path = tmp_path / ".zoot" / "memory" / "topics" / "key-decisions.md"
    report = json.loads(agent.run_store.report_path(agent.current_task_state).read_text(encoding="utf-8"))
    ...
    assert report["durable_promotions"] == [
        ...
        "project-conventions: Preserve local agent state under .zoot/.",
...
def test_final_memory_tags_are_appended_to_daily_log(tmp_path):
    ...
    log_files = list((tmp_path / ".zoot" / "memory" / "logs").rglob("*.md"))
    report = json.loads(agent.run_store.report_path(agent.current_task_state).read_text(encoding="utf-8"))
    ...
    assert report["memory_maintenance"]["memory_tags_appended"][0]["path"].startswith(".zoot/memory/logs/")
...
def test_auto_dream_runs_in_background_after_session_gate(tmp_path):
    for index in range(2):
        (tmp_path / ".zoot" / "sessions" / f"older-{index}.json").parent.mkdir(parents=True, exist_ok=True)
        (tmp_path / ".zoot" / "sessions" / f"older-{index}.json").write_text("{}", encoding="utf-8")
    agent = build_agent(
        tmp_path,
        [
            "<final><memory>Project: use repo-local memory.</memory></final>",
            '<tool>{"name":"read_file","args":{"path":".zoot/memory/MEMORY.md","start":1,"end":50}}</tool>',
            '<tool>{"name":"write_file","args":{"path":".zoot/memory/MEMORY.md","content":"# Durable Memory Index\\n\\n- [Project](topics/project.md): Project memory\\n"}}</tool>',
            "<final>Dreamed.</final>",
        ],
...
def test_background_auto_dream_failure_restores_lock_and_reports_error(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("zoot.features.memory.run_dream", fail_dream)
    agent = build_agent(
        tmp_path,
...
def test_explicit_memory_promotion_rejects_secret_shaped_and_transient_lines(tmp_path):
    ...
    report = json.loads(agent.run_store.report_path(agent.current_task_state).read_text(encoding="utf-8"))
    conventions_path = tmp_path / ".zoot" / "memory" / "topics" / "project-conventions.md"
    dependency_path = tmp_path / ".zoot" / "memory" / "topics" / "dependency-facts.md"
...
def test_public_api_exports_resolve_through_package_path():
    ...
    assert not hasattr(zoot_pkg, "ScriptedModelClient")
    assert not hasattr(providers_pkg, "ScriptedModelClient")
    assert Zoot is not None
    assert not hasattr(zoot_pkg, "OllamaModelClient")
    assert SessionStore is not None
    assert WorkspaceContext is not None
    assert Path(zoot_pkg.__file__).as_posix().endswith("/zoot/__init__.py")
...
def test_package_import_surface_includes_cli_entrypoints():
    assert callable(zoot_pkg.main)
    assert callable(zoot_pkg.build_agent)
    assert callable(zoot_pkg.build_arg_parser)


def test_module_execution_help_works():
    result = subprocess.run(
        [sys.executable, "-m", "zoot", "--help"],
        capture_output=True,
        text=True,
    )
