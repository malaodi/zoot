...
from zoot.evaluation.metrics import (
    _provider_profile,
...
def test_provider_profile_uses_project_toml_before_legacy_pico_env(tmp_path, monkeypatch):
    ...
    with patch.dict(
        os.environ,
        {
            "ZOOT_DEEPSEEK_API_KEY": "sk-legacy-deepseek",
            "ZOOT_DEEPSEEK_MODEL": "legacy-deepseek-model",
            "ZOOT_DEEPSEEK_API_BASE": "https://legacy.deepseek.example/anthropic",
        },
        clear=True,
    ):
...
def test_write_benchmark_core_report_marks_resume_safe_metrics(tmp_path):
    ...
    report_path = tmp_path / "docs" / "metrics" / "zoot-benchmark-core-report.md"
