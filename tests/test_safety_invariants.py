...
from zoot import cli as zoot_cli
from zoot.core.task_state import TaskState
...
def test_cli_build_agent_wires_secret_env_names_from_parser(tmp_path):
    ...
    with patch.dict(os.environ, {"GITHUB_PAT": "ghp-1", "GH_PAT": "ghp-2"}, clear=True), patch(
        ...
        args = zoot_cli.build_arg_parser().parse_args(
            [
                "--cwd",
                str(tmp_path),
                "--approval",
        ...
        agent = zoot_cli.build_agent(args)
...
def test_cli_build_agent_uses_default_configured_secret_names(tmp_path):
    ...
    with patch.dict(os.environ, {"GH_PAT": "ghp-default-1"}, clear=True), patch(
        ...
        args = zoot_cli.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--approval", "auto"])
        agent = zoot_cli.build_agent(args)
...
def test_cli_build_agent_loads_project_env_secrets_before_redaction_setup(tmp_path):
    ...
    (tmp_path / "README.md").write_text("demo\n", encoding="utf-8")
    (tmp_path / ".env").write_text("ZOOT_DEEPSEEK_API_KEY=sk-project-secret\n", encoding="utf-8")
    with patch.dict(os.environ, {}, clear=True), patch("zoot.cli.AnthropicCompatibleModelClient", DummyModelClient):
        args = zoot_cli.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--provider", "deepseek"])
        agent = zoot_cli.build_agent(args)
        assert agent.secret_env_summary()["secret_env_names"] == ["ZOOT_DEEPSEEK_API_KEY"]
...
def test_cli_build_agent_reads_secret_names_from_environment_config(tmp_path):
    ...
    with patch.dict(
        os.environ,
        {
            "ZOOT_CUSTOM_SECRET": "custom-secret-value",
            "ZOOT_SECRET_ENV_NAMES": "ZOOT_CUSTOM_SECRET",
        },
        clear=True,
    ), patch("zoot.cli.OpenAICompatibleModelClient", DummyModelClient):
        args = zoot_cli.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--approval", "auto"])
        agent = zoot_cli.build_agent(args)
        assert agent.secret_env_summary()["secret_env_names"] == ["ZOOT_CUSTOM_SECRET"]
...
def test_bound_tool_methods_call_tools_module(tmp_path):
    ...
    with patch("zoot.tools.registry.subprocess.run") as fake_run:
    ...
    assert agent.tool_run_shell.__func__.__module__ == "zoot.core.runtime"
