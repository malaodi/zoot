...
from zoot.features import skills as skillslib
...
def build_agent(tmp_path, outputs):
    ...
    return Zoot(
        ...
        workspace=workspace,
        session_store=SessionStore(tmp_path / ".zoot" / "sessions"),
...
def test_builtin_skills_are_available_in_context(tmp_path):
    ...
    assert prompt.index("Memory:") < prompt.index("Available skills:") < prompt.index("Relevant process memory:")
...
def test_prompt_documents_project_skill_frontmatter_contract(tmp_path):
    ...
    prompt = agent.prompt("Create .zoot/skills/audit/SKILL.md")

    assert "When creating Zoot skill files" in prompt
    assert ".zoot/skills/<name>/SKILL.md" in prompt
...
def test_project_skill_slash_invocation_runs_inline_session(tmp_path):
    skill_dir = tmp_path / ".zoot" / "skills" / "deploy"
...
---
Use target $ARGUMENTS from ${ZOOT_SKILL_DIR}.
""",
        encoding="utf-8",
    )
...
def test_memory_slash_commands_use_kairos_assets(tmp_path):
    ...
    (tmp_path / ".zoot" / "memory" / "MEMORY.md").write_text(
...
def test_dream_slash_command_consolidates_daily_log_into_memory_files(tmp_path):
    agent = build_agent(
        ...
        [
            ...
            '<tool>{"name":"write_file","args":{"path":".zoot/memory/topics/user-preferences.md","content":"# User Preferences\\n\\n## Notes\\n- Prefers concise reports.\\n"}}</tool>',
    ...
    assert "User preferences" in (tmp_path / ".zoot" / "memory" / "MEMORY.md").read_text(encoding="utf-8")
    assert "Prefers concise reports" in (tmp_path / ".zoot" / "memory" / "topics" / "user-preferences.md").read_text(encoding="utf-8")
...
def test_skill_frontmatter_metadata_and_argument_substitution(tmp_path):
    skill_dir = tmp_path / ".zoot" / "skills" / "audit"
    skill_dir.mkdir(parents=True)
    (skill_dir / "SKILL.md").write_text(
        """---
name: audit
...
def test_fork_skill_runs_in_isolated_session_and_records_completion(tmp_path):
    skill_dir = tmp_path / ".zoot" / "skills" / "inspect"
    skill_dir.mkdir(parents=True)
    (skill_dir / "SKILL.md").write_text(
        """---
name: inspect
...
def test_skill_allowed_tools_restricts_inline_execution(tmp_path):
    skill_dir = tmp_path / ".zoot" / "skills" / "readonly"
    skill_dir.mkdir(parents=True)
    (skill_dir / "SKILL.md").write_text(
        """---
name: readonly
...
def test_disable_model_invocation_skill_returns_expanded_prompt(tmp_path):
    skill_dir = tmp_path / ".zoot" / "skills" / "template"
    skill_dir.mkdir(parents=True)
    (skill_dir / "SKILL.md").write_text(
        """---
name: template
...
def test_cli_lists_skills_without_calling_model(tmp_path):
    ...
    result = subprocess.run(
        [sys.executable, "-m", "zoot", "--cwd", str(tmp_path)],
        input="/skills\n/exit\n",
        text=True,
        capture_output=True,
        env=env,
