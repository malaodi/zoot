...
from zoot.features.memory import (
    LayeredMemory,
    append_to_daily_log,
    build_dream_prompt,
...
def test_durable_memory_index_and_topic_notes_are_loaded_and_retrieved(tmp_path):
    ...
    (topics_dir / "project-conventions.md").write_text(
        ...
        "- Preserve local agent state under .zoot/.\n",
        encoding="utf-8",
    )
...
def test_dream_prompt_targets_repo_local_memory_assets(tmp_path):
    ...
    prompt = build_dream_prompt(memory_root, transcript_dir=str(tmp_path / ".zoot" / "sessions"), session_ids=["s1", "s2"])
...
def test_dream_prompt_uses_four_phase_filesystem_maintenance_flow(tmp_path):
    memory_root = tmp_path / ".zoot" / "memory"
    transcript_dir = tmp_path / ".zoot" / "sessions"
...
def test_session_scan_deduplicates_session_files_and_event_logs(tmp_path):
    sessions_dir = tmp_path / ".zoot" / "sessions"
