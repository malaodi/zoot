...
def test_patch_requires_prior_fresh_read_and_allows_after_read(tmp_path):
    ...
    rejected = agent.run_tool("patch_file", {"path": "README.md", "old_text": "world", "new_text": "zoot"})
    ...
    patched = agent.run_tool("patch_file", {"path": "README.md", "old_text": "world", "new_text": "zoot"})

    assert patched == "patched README.md"
    assert (tmp_path / "README.md").read_text(encoding="utf-8") == "hello zoot\n"
...
def test_rejected_patch_can_be_retried_after_informing_read(tmp_path):
    agent = build_agent(
        tmp_path,
        [
            '<tool name="patch_file" path="README.md"><old_text>world</old_text><new_text>zoot</new_text></tool>',
