...
def assistant_contents(app):
    from zoot.tui.widgets import AssistantMessage
...
async def wait_for_tool_card_status(app, pilot, status, attempts=40, delay=0.1):
    from zoot.tui.widgets import ToolCard
...
def test_cli_defaults_interactive_tty_mode_to_tui(monkeypatch):
    from zoot.cli import build_arg_parser, interaction_mode

    monkeypatch.setattr(
        "zoot.cli.sys.stdin", type("Stdin", (), {"isatty": lambda self: True})()
    )
...
def test_cli_uses_plain_repl_for_piped_stdin(monkeypatch):
    ...
    monkeypatch.setattr(
        "zoot.cli.sys.stdin", type("Stdin", (), {"isatty": lambda self: False})()
    )
    args = build_arg_parser().parse_args(["--cwd", "/tmp/workspace"])
...
def test_status_bar_shows_runtime_identity(tmp_path):
    from zoot.tui.widgets import StatusBar
...
def test_slash_command_registry_suggests_and_parses_subagent():
    from zoot.commands.slash import (
        parse_subagent_args,
        resolve_command,
        suggest_commands,
    )
...
async def test_tui_slash_suggestions_complete_partial_command(tmp_path):
    ...
    from zoot.tui.widgets import InputBar, SlashSuggestions

    app = ZootTuiApp(build_agent(tmp_path, []))
...
async def test_tui_help_command_uses_existing_repl_commands(tmp_path):
    from zoot.tui.app import ZootTuiApp
    from zoot.tui.widgets import InputBar

    agent = build_agent(tmp_path, [])
    app = ZootTuiApp(agent)
...
async def test_tui_hides_welcome_after_first_turn_so_chat_stays_visible(tmp_path):
    from zoot.tui.app import ZootTuiApp
    from zoot.tui.widgets import ChatLog, InputBar, WelcomeBanner
...
async def test_tui_chat_stream_uses_terminal_transcript_layout(tmp_path):
    from zoot.tui.app import ZootTuiApp
    from zoot.tui.widgets import AssistantMessage, ChatLog, InputBar, UserMessage
    ...
    async with app.run_test(size=(100, 20)) as pilot:
        ...
        bar.input.value = "你是谁"
        await pilot.press("enter")
        text = await wait_for_assistant(app, pilot, "我是 zoot。")
...
async def test_tui_renders_tool_card_result(tmp_path):
    from zoot.tui.app import ZootTuiApp
    from zoot.tui.widgets import InputBar, ToolCard
...
async def test_tui_approval_prompt_controls_risky_tool(tmp_path):
    from zoot.tui.app import ZootTuiApp
    from zoot.tui.widgets import ConfirmPrompt, InputBar
...
async def test_tui_ask_user_prompt_returns_selected_choice(tmp_path):
    from zoot.tui.app import ZootTuiApp
    from zoot.tui.widgets import AskUserPrompt, InputBar
