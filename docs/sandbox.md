...
zoot 的 `run_shell` 工具可以在 [bubblewrap](https://github.com/containers/bubblewrap) 沙盒里执行命令，把模型的影响范围控制在仓库目录里。
...
```bash
zoot --sandbox best_effort
zoot --sandbox required
```
...
- 在不熟悉的项目里跑 zoot：`--sandbox best_effort --approval ask`
