...
zoot 的记忆系统让 agent **跨 session 保持对项目的认知**。不是把整个对话历史塞回 prompt，而是分四层落地，每层有自己的生命周期。
...
模型在回答里裹一对 `<memory>` 标签，zoot 自动 append 到当天的 daily log。
...
后台启一个隔离的 zoot 实例（write_scope 限制在 `.zoot/memory/`），把 daily log + 最近 session ID 一起喂给模型，让它写 / 更新 topic 文件和 MEMORY.md。
...
模型也可以手动 `read_file .zoot/memory/topics/<name>.md` 读完整 topic。
...
```bash
zoot --no-auto-dream     # 只关 auto-dream，保留 /remember /dream
```

或者在 toml / 启动时设 `feature_flags.memory = false`，但**不推荐**——这是 zoot 区别于其他 coding agent 的核心能力。

## 文件级 freshness 保护

在 patch_file / write_file 之前，zoot 会检查"是否最近 read 过这个文件"（通过 sha256 freshness）。如果没读过就改，会被 `prior_read_required` 拒绝。这层保护和 memory feature flag **解耦**——即便 memory 关闭，read freshness 也仍然追踪，避免 agent 改盲文件。
...
| auto-dream 不触发 | 检查 `.zoot/memory/.consolidate-lock` 的 mtime，距上次 24h 没到 |
...
1. 第一次进项目：让 zoot 跑 `/skills`、看 README、用 `/remember` 写下 1-2 条该仓库的关键约定。
2. 每天工作结束：`/dream` 一次，把当天观察沉淀。
3. 切换分支或一段时间没用：直接 `zoot --resume latest`，让它从工作记忆 + topic 里恢复上下文。

记忆只在本地，**不会上传**。删除 `.zoot/memory/` 就回到第一次见你的状态。
