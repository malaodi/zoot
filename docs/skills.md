...
Skill 是一段写在 markdown 文件里的可复用 prompt，可以通过 `/skill-name [args]` 调用。zoot 把它展开成一次普通 session 请求，沿用同一套工具、审批和事件链路。
...
1. **内置 skill** — zoot 自带
2. **用户 skill** — `~/.zoot/skills/<name>/SKILL.md`
3. **项目 skill** — `<repo>/skills/<name>/SKILL.md` 或 `<repo>/.zoot/skills/<name>/SKILL.md`

## 自定义一个 skill

最小例子，新建 `~/.zoot/skills/deploy/SKILL.md`：
...
`$ARGUMENTS` 会被替换为 `staging`。`${ZOOT_SKILL_DIR}` 会被替换为 skill 文件所在目录的绝对路径。
...
- 用 `disable-model-invocation: true` 配合 `zoot --tui` 可以在不发请求的情况下预览 skill 展开后的 prompt
