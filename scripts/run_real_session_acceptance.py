#!/usr/bin/env python3
"""Run deterministic real-session acceptance scenarios for Zoot."""
...
from zoot.testing import ScriptedModelClient  # noqa: E402
from zoot import Zoot, SessionStore, WorkspaceContext  # noqa: E402
from zoot.config import resolve_provider_config  # noqa: E402
from zoot.features.skills_runtime import invoke_skill  # noqa: E402
from zoot.providers import AnthropicCompatibleModelClient, OpenAICompatibleModelClient, ProviderError  # noqa: E402

SUMMARY_JSON = "gate8-real-session-acceptance.json"
SUMMARY_MARKDOWN = "gate8-real-session-acceptance.md"
LIVE_ENV_FLAG = "ZOOT_ACCEPTANCE_LIVE"
...
def _scenario_plan_todo_explore(output_dir, workspace):
    ...
    agent = _build_agent(
        ...
        [
            ...
            '<tool name="write_file" path=".zoot/plans/gate8-plan.md"><content># Gate8 Plan\n- Evidence harness\n</content></tool>',
    ...
    return _finalize(
        ...
        checks=[
            ...
            _check("plan_file", (workspace / ".zoot" / "plans" / "gate8-plan.md").is_file()),
...
def _scenario_skill_inline(output_dir, workspace):
    ...
    skill_dir = workspace / ".zoot" / "skills" / "evidence"
...
def _scenario_resume_continuation(output_dir, workspace):
    ...
    store = SessionStore(workspace / ".zoot" / "sessions")
    first = Zoot(
...
def _scenario_security_rejection(output_dir, workspace):
    ...
    old_secret = os.environ.get("ZOOT_ACCEPTANCE_SECRET")
    os.environ["ZOOT_ACCEPTANCE_SECRET"] = "zoot-secret-value-123"
    agent = _build_agent(
        workspace,
        [
            ...
            '<tool>{"name":"run_shell","args":{"command":"echo $ZOOT_ACCEPTANCE_SECRET","timeout":5}}</tool>',
    ...
    try:
        ...
        return _finalize(
            ...
            checks=[
                ...
                _check("secret_redacted", "zoot-secret-value-123" not in trace_text),
            ],
        )
    finally:
        if old_secret is None:
            os.environ.pop("ZOOT_ACCEPTANCE_SECRET", None)
        else:
            os.environ["ZOOT_ACCEPTANCE_SECRET"] = old_secret
...
def _scenario_provider_error_recovery(output_dir, workspace):
    ...
    agent = Zoot(
        ...
        workspace=_scenario_workspace(workspace),
        session_store=SessionStore(workspace / ".zoot" / "sessions"),
        approval_policy="auto",
        max_steps=2,
    )
...
def _scenario_live_provider_smoke(output_dir, workspace, include_live=None):
    ...
    agent = Zoot(
        ...
        max_steps=1,
        max_new_tokens=64,
        secret_env_names=["ZOOT_API_KEY", "OPENAI_API_KEY", "ANTHROPIC_API_KEY", "DEEPSEEK_API_KEY"],
    )
...
def build_arg_parser():
    parser = argparse.ArgumentParser(description="Run Zoot Gate8 deterministic real-session acceptance scenarios.")
