#!/usr/bin/env python3
"""Run business-shaped Zoot dogfood scenarios against a real provider."""
...
from zoot.providers import AnthropicCompatibleModelClient, OpenAICompatibleModelClient  # noqa: E402
...
def render_markdown(summary):
    ...
    lines = [
        "# Zoot Business Scenario Dogfood",
        "",
...
def _build_client_factory(*, config_path=None, provider=None, model=None, base_url=None, api_key=None):
    ...
    if not config.api_key:
        raise ValueError(f"provider {config.name!r} has no api key; configure .zoot.toml or pass --api-key")
...
def build_arg_parser():
    parser = argparse.ArgumentParser(description="Run Zoot business scenario dogfood against a real provider.")
    parser.add_argument("--output-dir", default="/tmp/zoot-business-scenario-dogfood", help="Directory for workspaces and summary artifacts.")
    parser.add_argument("--config", default=None, help="Path to a Zoot TOML config file.")
    ...
    parser.add_argument("--max-steps", type=int, default=8, help="Max Zoot steps per scenario turn.")
