#!/usr/bin/env python3
"""Run the prioritized Zoot v3 human-scenario gate.
 
The runner intentionally drives Zoot through its public CLI entrypoint. It does
not import the Zoot runtime; verification reads only the files Zoot writes.
"""
...
from zoot.evaluation.run_evidence import RunEvidence


ROOT = Path(__file__).resolve().parents[1]
DEFAULT_CONFIG = ROOT / ".zoot.toml"
...
class HumanScenarioRunner:
    def __init__(self, args: argparse.Namespace):
        ...
        self.output_dir = Path(args.output_dir or Path("/tmp") / "zoot-v3-human-scenarios" / stamp).resolve()
        if _is_relative_to(self.output_dir, ROOT):
            raise ValueError(
                "output-dir must be outside the Zoot repo; otherwise Zoot discovers "
    ...
    def r01_student_management(self) -> ScenarioResult:
        ...
        command = self.run_zoot("R01", workspace, prompt=prompt, max_steps=10, max_new_tokens=2048, timeout=420)
        checks = [
    ...
    def r02_order_pricing_bugfix(self) -> ScenarioResult:
        ...
        command = self.run_zoot("R02", workspace, prompt=prompt, max_steps=9, max_new_tokens=1536, timeout=420)
        checks = [
            check("command_exit_0", command.returncode == 0),
    ...
    def r03_release_readiness_skill(self) -> ScenarioResult:
        ...
        skill_dir = workspace / ".zoot" / "skills" / "release"
...
""",
        ...
        command = self.run_zoot("R03", workspace, repl_input="/release billing-api\n/exit\n", max_steps=8, max_new_tokens=1536, timeout=420)
    ...
    def r04_incident_resume_fix(self) -> ScenarioResult:
        ...
        first = self.run_zoot("R04-first", workspace, prompt=first_prompt, max_steps=6, max_new_tokens=1536, timeout=360)
    ...
    def s07_repl_help(self) -> ScenarioResult:
        ...
        command = self.run_zoot("S07", workspace, repl_input="/help\n/exit\n", timeout=120)
    ...
    def s06_tty_default_tui(self) -> ScenarioResult:
        ...
        command = self.run_zoot_tty_smoke("S06", workspace, timeout=6)
        stdout = self.read_log(command.stdout_path)
        ...
        checks = [
            ...
            check("mentions_zoot_or_tui", "zoot" in (stdout + stderr).lower() or "Textual" in stdout + stderr),
        ]
    ...
    def s09_piped_stdin_repl(self) -> ScenarioResult:
        ...
        command = self.run_zoot("S09", workspace, repl_input="/help\n/exit\n", timeout=120)
        stdout = self.read_log(command.stdout_path)
        checks = [
            check("command_exit_0", command.returncode == 0),
    ...
    def s10_slash_suggestion_registry(self) -> ScenarioResult:
        ...
        command = self.run_python(
            "S10",
            workspace,
            "from zoot.commands.slash import suggest_commands\n"
    ...
    def s11_session_status(self) -> ScenarioResult:
        ...
        command = self.run_zoot("S11", workspace, repl_input="/plan refactor-auth\n/session\n/exit\n", timeout=120)
        stdout = self.read_log(command.stdout_path)
        checks = [
            check("command_exit_0", command.returncode == 0),
    ...
    def s12_usage_metadata(self) -> ScenarioResult:
        ...
        second = self.run_zoot("S12-usage", workspace, repl_input="/usage\n/exit\n", extra=["--resume", "latest"], timeout=120)
    ...
    def s13_model_runtime_switch(self) -> ScenarioResult:
        ...
        command = self.run_zoot("S13", workspace, repl_input="/model gpt-test-local\n/model\n/exit\n", timeout=120)
        stdout = self.read_log(command.stdout_path)
        checks = [
            check("command_exit_0", command.returncode == 0),
            check("model_switched", "model: gpt-test-local" in stdout),
            check("no_workspace_config_written", not (workspace / ".zoot.toml").exists()),
        ]
    ...
    def s14_clear_new_session(self) -> ScenarioResult:
        ...
        command = self.run_zoot("S14", workspace, repl_input="/session\n/clear\n/session\n/exit\n", timeout=120)
        stdout = self.read_log(command.stdout_path)
    ...
    def s15_plan_mode_active_artifact(self) -> ScenarioResult:
        ...
        prompt = (
            ...
            "如果被拒绝，再写 active plan artifact .zoot/plans/auth-refactor-plan.md，内容为 # Plan。最后 final。"
        )
        repl = f"/plan auth-refactor\n{prompt}\n/exit\n"
        command = self.run_zoot("S15", workspace, repl_input=repl, max_steps=5, max_new_tokens=1536, timeout=420)
        commands = [command]
        plan_path = workspace / ".zoot" / "plans" / "auth-refactor-plan.md"
        if not plan_path.is_file():
            continue_prompt = (
                "继续上一个 plan mode。不要再写 src/auth.py。"
                "只调用一次 write_file，path 必须是 .zoot/plans/auth-refactor-plan.md，"
    ...
    def s16_plan_final_gate(self) -> ScenarioResult:
        ...
        prompt = (
            ...
            "再 write_file .zoot/plans/cache-plan.md，内容为 # Cache Plan，然后 final。"
        )
        command = self.run_zoot("S16", workspace, repl_input=f"/plan cache\n{prompt}\n/exit\n", max_steps=5, max_new_tokens=1024, timeout=360)
        stdout = self.read_log(command.stdout_path)
        plan_path = workspace / ".zoot" / "plans" / "cache-plan.md"
        checks = [
            check("command_exit_0", command.returncode == 0),
    ...
    def s17_absolute_plan_path(self) -> ScenarioResult:
        ...
        absolute_plan = workspace / ".zoot" / "plans" / "student-plan.md"
        prompt = "write_file .zoot/plans/student-plan.md，内容为 # Student Plan，然后 final。"
        command = self.run_zoot(
            "S17",
            workspace,
        ...
        checks = [
            check("command_exit_0", command.returncode == 0),
            check("absolute_path_normalized", "plan path: .zoot/plans/student-plan.md" in stdout),
    ...
    def s18_plan_path_escape_rejected(self) -> ScenarioResult:
        ...
        command = self.run_zoot("S18", workspace, repl_input="/plan student .zoot/plans/../escape.md\n/exit\n", timeout=120)
        stdout = self.read_log(command.stdout_path)
        checks = [
            check("command_exit_0", command.returncode == 0),
            check("escape_rejected", "plan path must stay under .zoot/plans/" in stdout),
            check("outside_not_written", not (workspace / ".zoot" / "escape.md").exists()),
        ]
    ...
    def s19_plan_allows_explore(self) -> ScenarioResult:
        ...
        command = self.run_zoot("S19", workspace, repl_input=f"/plan payments\n{prompt}\n/exit\n", max_steps=4, max_new_tokens=1536, timeout=420)
    ...
    def s21_prior_read_required(self) -> ScenarioResult:
        ...
        prompt = (
            ...
            "1) 先尝试 patch_file README.md old_text='world' new_text='zoot'，不要先 read_file。"
            "2) 如果被拒绝，再 read_file README.md start=1 end=1。"
            "3) 再 patch_file README.md old_text='world' new_text='zoot'。"
            "4) final。"
        )
        command = self.run_zoot("S21", workspace, prompt=prompt, max_steps=7, max_new_tokens=1536, timeout=420)
        text = (workspace / "README.md").read_text(encoding="utf-8")
        checks = [
            check("command_exit_0", command.returncode == 0),
            check("readme_patched_after_read", text == "hello zoot\n", text),
        ]
    ...
    def s22_new_file_then_overwrite_requires_read(self) -> ScenarioResult:
        ...
        command = self.run_zoot("S22", workspace, prompt=prompt, max_steps=8, max_new_tokens=1536, timeout=420)
    ...
    def s23_self_authored_patch(self) -> ScenarioResult:
        ...
        command = self.run_zoot("S23", workspace, prompt=prompt, max_steps=5, max_new_tokens=1024, timeout=300)
    ...
    def s24_shell_search_denied(self) -> ScenarioResult:
        ...
        command = self.run_zoot("S24", workspace, prompt=prompt, max_steps=5, max_new_tokens=1024, timeout=300)
        checks = [
            check("command_exit_0", command.returncode == 0),
    ...
    def s25_pipe_output_management_allowed(self) -> ScenarioResult:
        ...
        command = self.run_zoot("S25", workspace, prompt=prompt, max_steps=3, max_new_tokens=768, timeout=240)
    ...
    def s26_long_shell_output_artifact(self) -> ScenarioResult:
        ...
        command = self.run_zoot("S26", workspace, prompt=prompt, max_steps=3, max_new_tokens=1024, timeout=240)
    ...
    def s27_approval_never_rejects_risky_tool(self) -> ScenarioResult:
        ...
        command = self.run_zoot("S27", workspace, prompt=prompt, approval="never", max_steps=3, max_new_tokens=768, timeout=240)
        checks = [
            check("command_exit_0", command.returncode == 0),
    ...
    def s30_skills_list_local(self) -> ScenarioResult:
        ...
        command = self.run_zoot("S30", workspace, repl_input="/skills\n/exit\n", timeout=120)
        stdout = self.read_log(command.stdout_path)
        checks = [
            check("command_exit_0", command.returncode == 0),
            check("skills_listed", "/review" in stdout and "/test" in stdout and "/commit" in stdout),
            check("no_run_created", not any((workspace / ".zoot" / "runs").glob("run_*")) if (workspace / ".zoot" / "runs").exists() else True),
        ]
    ...
    def s32_project_skill_arguments(self) -> ScenarioResult:
        ...
        skill_dir = workspace / ".zoot" / "skills" / "deploy"
        skill_dir.mkdir(parents=True)
        (skill_dir / "SKILL.md").write_text(
            """---
name: deploy
...
---
请只返回 <final>deploy checked $ARGUMENTS from ${ZOOT_SKILL_DIR}</final>，不要调用工具。
""",
            encoding="utf-8",
        )
        command = self.run_zoot("S32", workspace, repl_input="/deploy staging\n/exit\n", max_steps=3, max_new_tokens=512, timeout=240)
        stdout = self.read_log(command.stdout_path)
        checks = [
            check("command_exit_0", command.returncode == 0),
    ...
    def s33_skill_allowed_tools_restricts_write(self) -> ScenarioResult:
        ...
        skill_dir = workspace / ".zoot" / "skills" / "readonly"
        skill_dir.mkdir(parents=True)
        (skill_dir / "SKILL.md").write_text(
            """---
name: readonly
...
""",
        ...
        command = self.run_zoot("S33", workspace, repl_input="/readonly now\n/exit\n", max_steps=5, max_new_tokens=1024, timeout=300)
        checks = [
            check("command_exit_0", command.returncode == 0),
    ...
    def s34_fork_skill_keeps_parent_history(self) -> ScenarioResult:
        ...
        skill_dir = workspace / ".zoot" / "skills" / "inspect"
        skill_dir.mkdir(parents=True)
        (skill_dir / "SKILL.md").write_text(
            """---
name: inspect
...
""",
        ...
        first = self.run_zoot("S34-first", workspace, prompt="不要调用工具，直接返回 <final>parent seed</final>。", max_steps=2, max_new_tokens=512, timeout=180)
        second = self.run_zoot("S34-skill", workspace, repl_input="/inspect README.md\n/exit\n", extra=["--resume", "latest"], max_steps=2, max_new_tokens=512, timeout=240)
    ...
    def s35_prompt_only_skill(self) -> ScenarioResult:
        ...
        skill_dir = workspace / ".zoot" / "skills" / "template"
        skill_dir.mkdir(parents=True)
        (skill_dir / "SKILL.md").write_text(
            """---
name: template
...
""",
        ...
        command = self.run_zoot("S35", workspace, repl_input="/template world\n/exit\n", timeout=120)
        stdout = self.read_log(command.stdout_path)
        checks = [
            check("command_exit_0", command.returncode == 0),
    ...
    def s36_invalid_skill_frontmatter_diagnostic(self) -> ScenarioResult:
        ...
        skill_dir = workspace / ".zoot" / "skills" / "bad"
        skill_dir.mkdir(parents=True)
        (skill_dir / "SKILL.md").write_text("---\nname: bad\n---\nBad skill still loadable.\n", encoding="utf-8")
        command = self.run_zoot("S36", workspace, repl_input="/skills\n/exit\n", timeout=120)
        stdout = self.read_log(command.stdout_path)
        checks = [
            check("command_exit_0", command.returncode == 0),
    ...
    def s37_explore_subagent(self) -> ScenarioResult:
        ...
        command = self.run_zoot("S37", workspace, prompt=prompt, max_steps=5, max_new_tokens=1536, timeout=420)
    ...
    def s38_worker_write_scope(self) -> ScenarioResult:
        ...
        command = self.run_zoot("S38", workspace, prompt=prompt, max_steps=4, max_new_tokens=1536, timeout=420)
        report = self.evidence(workspace).report
        workers = ((report.get("workers") or {}).get("items") or [])
        checks = [
            check("command_exit_0", command.returncode == 0),
    ...
    def s39_worker_continuation(self) -> ScenarioResult:
        ...
        command = self.run_zoot("S39", workspace, prompt=prompt, max_steps=6, max_new_tokens=1536, timeout=540)
        report = self.evidence(workspace).report
        workers = ((report.get("workers") or {}).get("items") or [])
        checks = [
            check("command_exit_0", command.returncode == 0),
    ...
    def s40_running_worker_send_guard(self) -> ScenarioResult:
        ...
        command = self.run_zoot("S40", workspace, prompt=prompt, max_steps=5, max_new_tokens=1536, timeout=420)
        trace_text, report_text = self.latest_trace_and_report_text(workspace)
        checks = [
            check("command_exit_0", command.returncode == 0),
    ...
    def s41_task_stop_worker(self) -> ScenarioResult:
        ...
        command = self.run_zoot("S41", workspace, prompt=prompt, max_steps=5, max_new_tokens=1536, timeout=420)
        report = self.evidence(workspace).report
        workers = ((report.get("workers") or {}).get("items") or [])
        checks = [
            check("command_exit_0", command.returncode == 0),
    ...
    def s42_clear_stops_worker(self) -> ScenarioResult:
        ...
        second = self.run_zoot("S42-clear", workspace, repl_input="/clear\n/agents\n/exit\n", extra=["--resume", "latest"], timeout=120)
        stdout = self.read_log(second.stdout_path)
        checks = [
    ...
    def s43_remember_daily_log(self) -> ScenarioResult:
        ...
        command = self.run_zoot("S43", workspace, repl_input="/remember 这个项目用 pytest，不用 unittest\n/exit\n", timeout=120)
        logs = list((workspace / ".zoot" / "memory" / "logs").rglob("*.md"))
    ...
    def s44_dream_writes_memory(self) -> ScenarioResult:
        ...
        repl = (
            ...
            "/remember Decision: keep Zoot artifacts under .zoot/runs\n"
            "/remember Project convention: prefer small files\n"
            "/dream\n"
            "/exit\n"
        )
        command = self.run_zoot("S44", workspace, repl_input=repl, max_steps=5, max_new_tokens=2048, timeout=420)
        memory_root = workspace / ".zoot" / "memory"
    ...
    def s45_secret_memory_rejected(self) -> ScenarioResult:
        ...
        command = self.run_zoot("S45", workspace, prompt=prompt, max_steps=2, max_new_tokens=1024, timeout=240)
        memory_root = workspace / ".zoot" / "memory"
        memory_text = "\n".join(path.read_text(encoding="utf-8") for path in memory_root.rglob("*.md")) if memory_root.exists() else ""
        report = self.evidence(workspace).report
        checks = [
    ...
    def s50_path_traversal_and_redaction(self) -> ScenarioResult:
        ...
        env = dict(os.environ)
        env["ZOOT_HUMAN_SECRET"] = secret
        command = self.run_zoot(
            "S50",
            workspace,
            prompt=prompt,
            extra=["--secret-env-name", "ZOOT_HUMAN_SECRET"],
            env=env,
            max_steps=5,
            max_new_tokens=1536,
            timeout=360,
    ...
    def run_zoot_tty_smoke(self, name: str, workspace: Path, *, timeout: int = 6) -> CommandRecord:
        args = [
            "uv",
            "run",
            "zoot",
    ...
    def latest_events_path(self, workspace: Path) -> Path | None:
        events = sorted((workspace / ".zoot" / "sessions").glob("*.events.jsonl"), key=lambda path: path.stat().st_mtime)
    ...
    def latest_session_id(self, workspace: Path) -> str:
        sessions = sorted((workspace / ".zoot" / "sessions").glob("*.json"), key=lambda path: path.stat().st_mtime)
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Zoot v3 human-scenario release gate.")
    ...
    parser.add_argument("--config", default=str(DEFAULT_CONFIG), help="Zoot config file. Defaults to this repo's ignored .zoot.toml.")
    parser.add_argument("--provider", default="deepseek", help="Provider profile to pass to Zoot.")
